...
public class Network {
	...
	public static ArrayList<Double> arrError;
	public static ArrayList<Integer> epoch;

	static public void Init(String[] args) {
		arrError = new ArrayList<Double>();
		epoch = new ArrayList<Integer>();
		try {
		...
		while (numTimes < 1000) {
			...
			if (numTimes == 0) {
				arrError.add(realError);
				epoch.add(numTimes);
			}
			if (numTimes % 10 == 0) {
				arrError.add(realError);
				epoch.add(numTimes);
				System.out.println("Error is " + realError + " ----- Epoch is "
						+ numTimes);
			}
		}
	...
	public static void testFase() {
		...
		System.out.println("Correct output was achieved " + hitRatio * 100.0
				+ "% of the times");
	}
			...
			for (int j = 0; j < network[i].length; j++) {
				for (int k = 0; k < network[i][j].dweights.length; k++) {
					...
					if (network[i][j].prevDeltas != null) {
						network[i][j].dweights[k] -= (network[i][j].learningRate
								* deltaW + network[i][j].prevDeltas[k]
								* momentum);
						network[i][j].prevDeltas[k] = network[i][j].learningRate
								* deltaW;
					} else {
				...
				if (network[i][j].prevBiasDelta != 2) {
					network[i][j].biasWeight -= (network[i][j].learningRate
							* network[i][j].gradient * network[i][j].bias + momentum
							* network[i][j].prevBiasDelta);
					network[i][j].prevBiasDelta = network[i][j].learningRate
							* network[i][j].gradient * network[i][j].bias;
				} else {
					network[i][j].prevBiasDelta = network[i][j].learningRate
							* network[i][j].gradient * network[i][j].bias;
					network[i][j].biasWeight -= network[i][j].learningRate
	...
	static void randomInput() {
		...
		while (inputCopy.size() != 0) {
			...
			if (turn) {
				...
				randomPos = rand.nextInt(((inputCopy.size() - 1) - ((inputCopy
						.size() - 1) / 2)) + 1) + (inputCopy.size() - 1) / 2;
				if (rand.nextBoolean() == true)
					turn = true;
			}
			...
			target[0] = inputCopy.get(randomPos).get(
					input.get(randomPos).size() - 1);
			target[1] = inputCopy.get(randomPos).get(
					input.get(randomPos).size() - 2) / 100.0;
			forward();
			backPropagation();
